...
def possede(donnees_personnage, type_caracteristique, valeur_caracteristique):
    ...
    """
    if type_caracteristique in donnees_personnage:
...
def score_dichotomie(personnages_restants, type_caracteristique, valeur_caracteristique):
    ...
    Returns:
        int: Le score.
    """
    avec_caracteristique = 0
    sans_caracteristique = 0

    for personnage in personnages_restants.values():
        if possede(personnage, type_caracteristique, valeur_caracteristique):
            avec_caracteristique += 1
        else:
            sans_caracteristique += 1

    total_personnages = avec_caracteristique + sans_caracteristique
    score = total_personnages - max(avec_caracteristique, sans_caracteristique)
    return score
...
def selectionner_caracteristique(personnages_restants):
    ...
    for type_carac in types_caracteristiques_ordre_aleatoire():
        for valeur in valeurs_ordre_aleatoire(type_carac):
            score = score_dichotomie(personnages_restants, type_carac, valeur)
            if score > meilleur_score:
                meilleur_score = score
                meilleur_type = type_carac
                meilleure_valeur = valeur

    return meilleur_type, meilleure_valeur
...
def mettre_a_jour_hypotheses(personnages_restants, type_caracteristique, valeur_caracteristique, reponse):
    ...
    for nom, caracteristiques in personnages_restants.items():
        if possede(caracteristiques, type_caracteristique, valeur_caracteristique) == reponse:
            personnages[nom] = caracteristiques
    return personnages
